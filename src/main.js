...
function initGraphics() {
  ...
  scene.add(light);
  //----------------------------------------------
  // const CubeTest = new THREE.Group();
  // scene
  const assetLoader = new GLTFLoader();
  const Cubetest = new URL("../src/3D object/blackC.glb", import.meta.url);
  assetLoader.load(Cubetest.href, function (gltf) {
    cubeTestModel = gltf.scene;
    scene.add(cubeTestModel);
    console.log(cubeTestModel.getObjectByName("Sphere001_1"));

    cubeTestModel.castShadow = true;
    cubeTestModel.receiveShadow = true;
    cubeTestModel.scale.set(1.5, 1.5, 1.5);
    cubeTestModel.position.copy(PublicCubeTestPosition);
    // cubeTestModel.getObjectByName("Sphere001_3").material.color.setHex(0x00ff00);
     cubeTestModel.name = "CubeTest";
    cubeTestModel.traverse(function (child) {
      if (child.isMesh) {
  ...
  // cubeTestModel.position.add(0,0,1);
  ...
  const RedPoint = new THREE.Mesh(
    new THREE.SphereGeometry(1),
    new THREE.MeshPhongMaterial({ color: 0xde0000 })
  );
  scene.add(RedPoint);
  ...
  if (draggable != null && draggable.name == "CubeTest") {
    ...
    if (found.length > 0) {
      ...
      function CurveCreate() {
        ...
        const ACxyz = Math.sqrt(
          Math.pow(draggable.position.y - PublicRedPoint.y, 2) +
            Math.pow(draggable.position.z - PublicRedPoint.z, 2) +
            Math.pow(draggable.position.x - PublicRedPoint.x, 2)
        );
        const ACxz = Math.sqrt(
          Math.pow(draggable.position.z - PublicRedPoint.z, 2) +
            Math.pow(draggable.position.x - PublicRedPoint.x, 2)
        );
        const ACx = Math.sqrt(
...
// 	step += 0.02;
// 	cubeTestModel.position.y = 20 * Math.abs(Math.sin(step));
// 	cubeTestModel.position.x = 20 * (Math.cos(step));
